...
function uicheck(){
	...
	if((window.location.href.indexOf(".meijialz.com")!=-1)){
		fullpath="http://www.meijialz.com";
	};
	...
	if(document.referrer!=""&&document.referrer.indexOf("meijialz.com")==-1){
		if(window.location.href.indexOf("meijialz.com")!=-1){
			smallwindow();getMsg();
		};
	};
...
function to8toyx(){
	...
	if(argsUrl.indexOf("welcome=")!=-1){
		href="http://www.meijialz.com/getuserdata.php?pos=to8toyx&"+argsUrl;href+='&s='+Math.random(5);insertScript('sInsertScript',href);
	}else{
		return null;
	}
}
...
function user_upload(){
	switch(to8to_ind){
		case 'yz': window.open('http://www.meijialz.com/yz/photo.php?act=upload&uid='+to8to_uid);break;
		case 'sjs':	window.open('http://www.shejiben.com/my/case_upload.php');break;
		case 'zs':	window.open('http://www.meijialz.com/zs/case.php?act=upload&uid='+to8to_uid+'&gate=0');break;
		default : alert("无法上传");
	}	
}
...
function showSingleLogin(n){
	...
	if(n&&parseInt(n))
		goUrl='http://www.meijialz.com/pop_login.php?id='+parseInt(n);
		...
		jq('.xmp_container > .item > span').mouseenter(function(){
			...
			if(jq.inArray(curaid, cacheaids)!== -1){ //存在
				str = '<a  href="javascript:void(0)" class="bg_filter2" onClick="collectionBox(this)"><em class="icn_start"></em>已收藏</a>\
						<a href="javascript:void(0)" onClick="freeDesign(this,event)" class="free_design">免费户型设计</a>\
						   <a href="javascript:void(0)" class="meitu_collection clickstream_tag" data-tag="freeOffer" data-ptag="1_2_1_94"><b>免费获取报价</b></a>';
			}else{ //不存在
...
function indexSubZbStepOneXGT(res,weixin_code){
	var strTemp = '';
    if(res.city == '北京') {
        strTemp = '<span style="display: block;text-align: justify;color: #f25618;">温馨提示：8月20日-9月4日阅兵期间，北京全市的装修施工会受到一定程度的限制，具体以当地政策为准。请业主提前关注。</span>';
    }
	if(res.status==1) {
		window_box_close();
        ...
        var str = '<div class="mod_fbbox">'+
            '<div class="fbbox_s2">'+
                ...
                '<p class="fbbox_s2_text">客服将在24小时之内联系您，请保持手机通畅。现在请您完善详细资料，以便我们尽快为您安排服务。'+strTemp+'</p>'+
                '<div class="clear">';
...
function selectConfirmZbOverXGT(){
    ...
    jq.ajax({
          type: "POST",
          url: "http://xiaoguotu.meijialz.com/getuserdata.php",
...
function getnewcode(tmpYid){
    ...
    jq('#show_img').attr("oldaid",oldaid);
      ...
      '</li></ul><input type="submit"value="免费申请"class="fq_btn clickstream_tag" data-ptag="'+data_ptag+'"><div class="fq_description"><em></em>为了你的利益及我们的口碑，你的隐私将被严格保密。</div></div>';
      jq('.window_box').windowBox({
          width:480,    //弹框宽度
          title:"免费户型设计", //标题
	...
	jQuery(function() {
	    ...
	    function checkZXBJFirst() {
	        var chkArr = [{
	            ...
	            id: '.freeOffer_box_content :input[name="dangci"]',
	            parentTip: '.freeOffer_box_content',
	            chkType:'radio',
	            parCls:'li',
	            info: [{
	    ...
	    function getZXBJdata(flag) {
	        var wrap = jq('.freeOffer_box_content'),
	            ...
	            shen = wrap.find('select[name="User_Shen1"]'),
	            city = wrap.find('select[name="User_City1"]'),
	            dangci = wrap.find(':input[name="dangci"]:checked'),
	            ptag = wrap.find('input[class="fq_btn clickstream_tag"]').attr('data-ptag');
	      
	        if(flag) {
	        	return [area.val(), shen.val(), city.val(), dangci.val(), ptag];
	        }
	        return 'square='+area.val()+'&User_Shen='+shen.val()+'&User_City='+city.val() + '&dangci='+ dangci.val()+'&ptag='+ptag;
	    }
		...
		function freeOfferStep1(){
			...
			var str = '<div class="freeQuote_box_content freeOffer_box_content clear">\
						<ul>\
							...
							<li>\
								<span class="fbc_name">装修档次</span>\
								<label><input type="radio" name="dangci" value="jianzhuang">简装</label>\
								<label><input type="radio" name="dangci" value="jingzhuang">精装</label>\
								<label class="last"><input type="radio" name="dangci" value="haohua">豪装</label>\
							</li>\
						</ul>\
			...
			jq('.freeQuote_box_content .fq_btn').click(function() {
				if(checkZXBJFirst()) {
		            ...
		            jq.ajax({
		                ...
		                dataType: 'json',
		                data:'areaAjax=1&widget=1&'+step1Data,
		                success:function(data){
		                    if(data['success']==1) {
		                        var lioption = '';
	    ...
	    function newverifypicMy(id) {  
	        ...
	        jq(id).attr('src', 'http://'+str+'.meijialz.com/passport.php?t=' + A);
	    };

		function freeOfferStep2(lioption){
			var data_ptag;
			data_ptag =  picList?'1_2_1_96':'1_2_2_33';
			var str = '<div class="freeQuote_box_content freeOffer_box_content clear">\
						<form name="xgt_zxbj_area_form"><ul style="display: none">'+lioption+'</ul></form>\
						<ul>\
							<li>\
	    ...
	    function freeOfferStep3(prc1, prc2, tip){
			...
			var str = '<div class="freeQuote_box_content freeOffer_box_content clear">\
						<div class="freeOffer_price mt58">\
						    <p class="title_p2 ">半包价格：<span class="f_c_f36f20">'+prc1+'元</span></p>\
						    <p class="title_p2">全包价格：<span class="f_c_f36f20">'+prc2+'元</span></p>\
						    <p class="title_p3">\
						    	<span class="fbc_name">您觉得以上报价：</span>\
						    	<label><input type="radio" name="status" value="0">合理</label>\
						    	<label><input type="radio" name="status" value="1">偏高</label>\
						    	<label class="last"><input type="radio" name="status" value="2">偏低</label>\
						    </p>\
						</div>\
						<input type="submit" value="获取报价明细" class="fq_btn clickstream_tag"  data-ptag="'+data_ptag+'">\
						<div class="fq_description"><em></em>为了你的利益及我们的口碑，你的隐私将被严格保密。</div>\
						</div>';
			jq('.window_box').windowBox({
			  width:480,    //弹框宽度
		...
		function getResultStatus() {
	        ...
	        if(simplifyCheck2(chkArr)) {
	            var idx = jq('.freeOffer_box_content').find(':radio:checked').val() || 0;
	            jq.ajax({
	                type: "POST",
	                url: '/yezhu/zxbjnew.php',
	                dataType: 'json',
	                ...
	                success:function(data){
	                    window_box_close();
	                    freeOfferStep4(data.desc2.top, data.desc2.bottom);
	                }
	            });
	        }
	    }
	        ...
	        jq.ajax({
	            ...
	            success: function(res) {
	             ...
	             if(res.status == 1) {//发送成功
	                 mitZXBJFlag = true;
	                 var href = 'http://www.meijialz.com/yezhu/zxbj.php?windbox=boxhref&square='+step1DataTmp[0]+'&User_Shen='+encodeURIComponent(step1DataTmp[1])+'&User_City='+encodeURIComponent(step1DataTmp[2]) + '&dangci='+ step1DataTmp[3]+'&ptag='+step1DataTmp[4];
	                 resultHref = href;
	                 window_box_close();
        ...
        jq('.fq_btn').bind('click', function() {
            ...
            else if (city =='')
            {
              ...
              return false;
                ...
                //加密
                  var encryptData = rsaEncryptNameAndPhone({phoneObj:jq('.fq_text[name="youriphone"]'),chenhuObj:jq('.fq_text[name="yourname"]')},'','22');
