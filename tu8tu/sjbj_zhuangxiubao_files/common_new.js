...
function newverifypic(id) {
    var id = id || 'passport';
		A = new Date().getTime();
    if ($(id)) {
        $(id).src = 'http://www.meijialz.com/passport.php?t=' + A;
    };
};
...
function UpdateStaticInfo() {
    ...
    for (i = 0; i < l; i++) {
        if (visitmember[i].ind == 'yz') {
            homepage = 'http://bbs.meijialz.com/space-uid-' + visitmember[i].vid + '.html';
        } else {
            homepage = "http://www.meijialz.com/" + visitmember[i].ind + "/" + visitmember[i].vid + "/";
        }
        if (who_ind == 0)
    ...
    function initsearch(sName)
        {
           ...
           switch(sName)
           {
                case '效果图': 
                   ...
                   sHref= 'http://xiaoguotu.meijialz.com/search.php';
                   sText ='海量精美效果图任你选'; 
                   tRight = 70;
                break;   
                case '装修公司':
                   ...
                   sHref= "http://"+tcode+".meijialz.com/company/";
                   sText ='挑选您心仪的装修公司';
                   tRight = 58;
                break;   
                case '小区':
                    ...
                    sHref=  "http://"+tcode+".meijialz.com/zwj/";
                    sText ='找找您家小区的装修案例';
                    tRight = 82;
                    break;   
                case '文章':
                    searchclicktage='1_8_2_4';
                    sHref= 'http://www.meijialz.com/yezhu/xzx_search.php';
                    sText ='了解装修相关的资讯知识';
                    tRight = 82;
                    break;   
                case '问答': 
                    ...
                    sHref= 'http://www.meijialz.com/ask/search.php';
                    sText ='解决您的装修疑问';
                    tRight = 82;
                    break;   
                case '家居建材': 
                    ...
                    sHref= 'http://mall.meijialz.com/search';
                    sText ='挑选优质家居建材'; 
                    tRight = 58;
                    break;    
           }
       ...
       obj.find('li').mouseenter(function(){
          ...
          if(jq(this).parent().attr('v') != '1'){
              jq("img.lazy1").lazyload({
                    placeholder : "http://img.meijialz.com/front_end/bg/grey.gif",effect : "fadeIn",failurelimit : 54
              });
          };
...
function indexSubZbStepOneNew(res, weixin_code, type, data) {//type: 1 设计与报价，2 装修公司，3 效果图
    var date = (new Date()).getHours(),
        tip = '';
    if(date >= 0 && date < 9) {
        tip = '今天上午将安排专属家装顾问优先为您服务。';
    } else if(date >= 9 && date < 12) {
        tip = '今天中午前将安排专属家装顾问优先为您服务。';
    } else if(date >= 12 && date < 18) {
        tip = '30分钟内将安排专属家装顾问优先为您服务。';
    } else {
        tip = '第二天上午将安排专属家装顾问优先为您服务。';
    }

    var sepcialtip = '',
        strTemp = '';
    if(res.city == '北京') {
        sepcialtip = 'sepcialtip';
        strTemp = '<span style="display: block;text-align: justify;">温馨提示：8月20日-9月4日阅兵期间，北京全市的装修施工会受到一定程度的限制，具体以当地政策为准。请业主提前关注。</span>';
    }
    var type = type || 1,
        weixin_code = type == 1 ? 'http://img.meijialz.com/to8to_img/to8to_wx_bj.png' : weixin_code;
    var str = '<form id="subZbForm">\
                ...
                <div class="mod_fbbox mod_fbbox_wxserviceV2 '+sepcialtip+'" style="width:308px;">\
                    <div class="fbbox_s1">\
                        <div class="s1_hd">恭喜您申请成功!</div>\
                        <div class="s1_hd_sub" style="width:100%;">由于近期申请火爆，“将在一个月内进行整屋装修”的业主，我们将优先服务，当然您也可以完善资料获取更快的审核服务，感谢您的理解和支持。'+strTemp+'</div>\
                    </div>\
                    <div class="mod_form">';
...
function weixinCodeTipBox(weixin_code, type, yid, data) {
    ...
    var str = '<div class="mod_fbbox mod_fbbox_wxserviceV2">\
                    ...
                    <div class="mod_fbbox_code">\
                        ...
                        <p>您还可以查看更多<a href="http://xiaoguotu.meijialz.com/" target="_blank">精品效果图</a><br>或者去<a href="http://mall.meijialz.com/" target="_blank">特卖商城</a>选购优质建材</p>\
                    </div>\
                </div>';
    jq('.window_box').windowBox({
        width: 530,
    ...
    if(res.status==1) {
        ...
        var str = '<div class="mod_fbbox">'+
            '<div class="fbbox_s2">'+
                ...
                '<p class="fbbox_s2_text" style="width:465px;">由于近期申请火爆，“将在一个月内进行整屋装修”的业主，我们将优先服务，当然您也可以完善资料获取更快的审核服务，感谢您的理解和支持。</p>'+
                '<div class="clear">';
...
jq(function() {
    ...
    function createUserNav() {
        ...
        str_havel='<div rel="nofollow" class="col_l htr_username_box"><a href="javascript:;" class="htr_username"><p class="ect">'+username+'</p><i class="triangle_down"></i></a><ul class="user_memu" style="display:none"><li><a href="http://www.meijialz.com/my/">个人中心</a></li>';

        if(ind=='yz'){
            str_havel += '<li><a href="http://www.meijialz.com/my/yz_administration_self.php?act=1" id="userbar-myinfo" class="">帐号设置</a></li>'; 
        }else if(ind=='zs'){ 
            str_havel += '<li><a href="http://www.meijialz.com/zs/'+uid+'">公司主页</a></li>';  
            str_havel += '<li><a href="http://www.meijialz.com/my/gs_data.php" >帐号设置</a></li>'; 
        };
        str_havel += '<li><a href="http://www.meijialz.com/logout.php?uid='+uid+'">退出</a></li></ul></div>';

        jq.ajax({  
            type:"GET",      
            dataType: 'jsonp',      
            url:"http://www.meijialz.com/api/get_message_count.php", 
...
var weChatQrcode = {
    ...
    createQrcodeTmp: function() {
        ...
        var qrcodeTmp = '<div class="wechat_barcode" id="wechat_barcode">\
                            <div class="we_inwrap">\
                            <a target="_blank" href="http://www.meijialz.com/apps/index.php?act=apps_wechat" rel="nofollow">\
