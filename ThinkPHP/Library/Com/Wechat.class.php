...
class Wechat {
    ...
    private function init(){
		...
		$mMap['show'] = '1';		
		$mRs = M('wexinacc')->field('appid,secret,type')->where($mMap)->find();
		if(empty($mRs)) return $this->replyText('没有此公众号'.$data['ToUserName']);
		self::$appId          = $mRs['appid'];
		self::$encodingAESKey = $mRs['secret'];
		if($mRs['type'] == 2) {
			session('wechat.type',2);
		}else{
			session('wechat.type',1);
		}
    ...
    public function replyNews($news, $news1, $news2, $news3){
		$tContent = func_get_args();
		foreach($tContent as $fKey => $fContent){
			if(empty($fContent)){
				unset($tContent[$fKey]);
			}
		}		
		
        return $this->response($tContent, self::MSG_TYPE_NEWS);
    }
